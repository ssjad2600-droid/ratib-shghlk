...
describe('🔴 تحديد النصّ ولوحة المفاتيح', () => {
  ...
  /**
   * 🔴 بيانات بطاقة المنتج تُقرأ على الهاتف.
   *
   * قِيس بالنظر على ٣٧٥px بعد تشغيل المحاكي: صفّا البطاقة يحملان **كل ما
   * يقرأه التاجر** — الكمية، وسعر الشراء والبيع، والربح — وكانا ١٠px. وشاشة
   * المخزون وحدها كان فيها ٦١ عنصراً بهذا الحجم، أي أكثر من نصف نصوصها.
   *
   * 🎯 والعلاج مشروطٌ بالمنصّة لا مطلق: `text-[11px] md:text-[10px]`. فالكمبيوتر
   * يبقى كما هو حرفياً — قِيس على ١٢٨٠px: ١٠px قبل وبعد — ويُرفع الهاتف وحده.
   * والصفّ `flex-wrap` فلا يتجاوز بل يلتفّ. النتيجة: ٦١ ⟵ ١١، وصفر تجاوز.
   *
   * وبقيّة الشاشات لم تُمَسّ عمداً: فحصتُها فوجدت الـ١٠px فيها تسمياتٍ وصفية
   * ثانوية («إيجار، كهرباء، رواتب…») وأرقامها الأساسية ٢٠–٢٤px — تدرّجٌ سليم
   * لا علّة.
   */
  it('🔴 بيانات بطاقة المنتج أكبر على الهاتف منها على الكمبيوتر', () => {
    const src = readCode('src/components/ProductsView.tsx');
    const rows = (src.match(/text-\[11px\] md:text-\[10px\]/g) ?? []).length;
    expect(rows, 'صفّا الكمية والأسعار كلاهما مشروط بالمنصّة').toBeGreaterThanOrEqual(2);
    expect(
      /font-black flex items-center gap-1 text-xs md:text-\[10px\]/.test(src),
      'الكمية أهمّ رقمٍ في الصفّ — ١٢px على الهاتف',
    ).toBe(true);
    // ولا يعود الصفّ إلى ١٠px مطلقاً بلا شرط منصّة
    expect(
      /gap-2 text-\[10px\] text-slate-600 font-bold flex-wrap/.test(src),
      'عودةٌ إلى ١٠px مطلقاً تُرجع النصّ غير المقروء على الهاتف',
    ).toBe(false);
  });
});
