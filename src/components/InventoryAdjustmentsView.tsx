  ...
  return <div className="space-y-5" dir="rtl">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200">
        ...
        <p className="text-xl text-rose-700 font-black mt-2">{money(stats.lostValue)}</p>
        <p className="text-[10px] text-slate-600 font-bold mt-1">{toArabicDigits(stats.lostUnits)} وحدة مخصومة{stats.addedUnits > 0 ? ` · ${toArabicDigits(stats.addedUnits)} مضافة` : ''}</p>
        ...
        {stats.reversedCount > 0 && <p className="text-[10px] text-slate-600 font-bold mt-1 leading-relaxed">{toArabicDigits(stats.reversedCount)} قيد متراجَع عنه — مستثنى من الحساب وظاهر في السجل</p>}
      </div>
      <div className="bg-white p-4 rounded-2xl border border-slate-200"><span className="text-xs text-slate-500 font-bold">أصناف متاحة للتسوية</span><p className="text-xl text-emerald-700 font-black mt-2">{toArabicDigits(products.length)}</p></div>
    </div>
